...
struct AnalyzerParams {
  ...
  AnalyzerParams(Registry &reg) {
    ...
    pregain = GainControlParams("input_gain", vg, FilterParams(1000.0, 1.0),
                                0.01, 0.00, -0.01);

    preemphasis = make_shared<float>(8.0);
    vg.newFloatValue("preemphasis", preemphasis, 1.0, 16.0, 0.1);

    normalizer = GainControlParams("normalizer", vg, FilterParams(600.0, 1.0),
                                   0.001, 0.0, -0.005);

    amp_filter = make_shared<FilterParams>(3.0, 1.0);
    amp_feedback = make_shared<FilterParams>(600.0, -1.0);
...
struct AnalyzerOutput {
  ...
  AnalyzerOutput(size_t size) : value(size), diff(size), energy(size) {}
};
...
public:
  Analyzer(size_t buffer_size, size_t fft_size, size_t bins, Registry &reg)
      : sfft(buffer_size, fft_size),
        bucketer(fft_size / 2, bins, 32.0, 16000.0), fft_buffer(fft_size),
        freq_bins(bins), params(reg), normalizer(bins), amp_filter(bins),
        amp_feedback(bins), diff_filter(bins), diff_feedback(bins),
        output(bins) {}
